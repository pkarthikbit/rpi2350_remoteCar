...
#define MQTT_SERVER "mqtt.inveh.in"
...
static void mqtt_incoming_subscribe_cb(void *arg, const char *topic, u32_t tot_len) 
{
    MQTT_CLIENT_DATA_T* state = (MQTT_CLIENT_DATA_T*)arg;
...
#endif
    mqtt_set_inpub_callback(state->mqtt_client_inst, mqtt_incoming_subscribe_cb, mqtt_incoming_data_cb, state);
    cyw43_arch_lwip_end();
}
...
static MQTT_CLIENT_DATA_T* mqtt_client_init(void) {
    MQTT_CLIENT_DATA_T *state = calloc(1, sizeof(MQTT_CLIENT_DATA_T));
    if (!state) {
    ...
    switch (rpi2350_rc_wifi_st)
    {
        case DEVICE_WIFI_START_UP:
        {
            ...
            {     
                ...
                if (err == ERR_OK) 
                {
                    //Do nothing as the callback will take care further
                } 
